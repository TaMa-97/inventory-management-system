...
// 商品一覧の取得
productRouter.get('/', (_: Request, res: Response) => {
...
// 在庫数の調整
productRouter.post('/:id/stock', (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
...
// 在庫移動履歴の取得
productRouter.get('/:id/stock-movements', (req: Request, res: Response) => {
  const id = Number(req.params.id);
